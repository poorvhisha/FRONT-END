        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          <div className="card card-sm">
            ...
            {[['✉️',emp.email],['📞',form.phone],['🏢',emp.department],['🪪',emp.id],['📅',`Joined ${formatDate(emp.joiningDate)}`]].map(([icon,val])=>(
              <div key={val} style={{display:'flex',gap:10,marginBottom:10,fontSize:13,color:'var(--text-muted)'}}><span>{icon}</span><span style={{wordBreak:'break-all'}}>{val}</span></div>
            ))}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:16}}>
          <div className="card">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              ...
              <Button variant={editing?'ghost':'outline'} size="sm" onClick={()=>editing?(toast.success('Profile updated!'),setEditing(false)):setEditing(true)}>{editing?'✓ Save':'✏️ Edit'}</Button>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              {[['Employee ID',emp.id],['Full Name',emp.name],['Email',emp.email],['Gender',emp.gender],['Date of Birth',formatDate(emp.dob)],['Joining Date',formatDate(emp.joiningDate)],['Department',emp.department],['Designation',emp.designation]].map(([l,v])=>(
              ...
              <div style={{padding:'10px 14px',background:'#F8FAFC',borderRadius:8,border:'1px solid var(--border)'}}>
                ...
                {editing ? <input className="form-input" style={{padding:'4px 8px',fontSize:13}} value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))}/> : <div style={{fontSize:14,fontWeight:500}}>{form.phone}</div>}
              </div>
              <div style={{padding:'10px 14px',background:'#F8FAFC',borderRadius:8,border:'1px solid var(--border)'}}>
                ...
                {editing ? <input className="form-input" style={{padding:'4px 8px',fontSize:13}} value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value}))}/> : <div style={{fontSize:14,fontWeight:500}}>{form.address}</div>}
              </div>
            </div>
          </div>
          <div className="card">
            <h3 style={{fontSize:16,marginBottom:16}}>Bank & Document Details</h3>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              {[['Bank Account',emp.bankAccount],['IFSC Code',emp.ifsc],['PAN Number',emp.pan],['Aadhaar',emp.aadhaar],['Emergency Contact',emp.emergencyContact]].map(([l,v])=>(
                <div key={l} style={{padding:'10px 14px',background:'#F8FAFC',borderRadius:8,border:'1px solid var(--border)'}}>
                  <div style={{fontSize:10,color:'var(--text-muted)',fontWeight:600,textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4}}>{l}</div>
                  <div style={{fontSize:14,fontWeight:500,fontFamily:'monospace'}}>{v||'-'}</div>
                </div>
              ))}
            </div>
          </div>
